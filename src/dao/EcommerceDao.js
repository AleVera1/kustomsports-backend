...
export default class EcommerceDAOMongo {
  ...
  deleteProdById = async (prodId) => {
    try {
      await this.db.connect();
      const result = await this.prodCollection.deleteOne({ _id: prodId });
      return result.deletedCount > 0 ? prodId : null;
    } catch (err) {
      console.log(err);
    }
  };
  ...
  getDetailedCart = async (req) => {
    try {
      ...
      const productsMongo = await this.cartCollection
        .findOne({ username: user }, { products: 1, _id: 0 })
        .lean();
      ...
      await Promise.all(
        productsArray.map(async (prod) => {
          const prodData = await this.prodCollection
            .findOne({ title: prod }, { price: 1, image: 1, _id: 0 })
            .lean();

          if (prodData) {
            // Add a check to ensure prodData is not null or undefined
            let prodInfo = {
              title: prod,
